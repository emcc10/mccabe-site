...
(function (global) {
  ...
  // MC_PDP_AUTH_DEPLOY_VERIFY_20260630sarrepair28
  ...
  var VERSION = "20260630sarrepair28";
  ...
  function applySaranoniInfoColumnAlignment() {
    ...
    if (!col) return;
    try {
      col.style.setProperty("margin-left", "10px", "important");
...
(function(global) {
  ...
  function fixInlineStyles() {
    console.log('[MC FIX CART] Running fixInlineStyles module');
    var cart = doc.querySelector('a.mc-cart-float, .mc-cart-float');
    if (cart) {
      console.log('[MC FIX CART] Cart width BEFORE:', cart.style.width);
      cart.style.width = '52px';
      cart.style.height = '52px';
      cart.style.minWidth = '52px';
      cart.style.maxWidth = '52px';
      cart.style.borderRadius = '999px';
      cart.style.display = 'flex';
      console.log('[MC FIX CART] Cart width AFTER:', cart.style.width, 'Computed:', getComputedStyle(cart).width);
    } else {
      console.log('[MC FIX CART] Cart element NOT FOUND');
    }
    ...
    if (relContent && relContent.style.display === 'none') {
      relContent.style.display = '';
      relContent.style.visibility = '';
    }
  }
  if (doc.readyState === 'loading') {
...
(function(global) {
  ...
  function centerAtcText() {
    ...
    if (atc) {
      atc.style.textAlign = 'center';
    }
  }
  if (doc.readyState === 'loading') {
    doc.addEventListener('DOMContentLoaded', centerAtcText);
  } else {
    centerAtcText();
  }
  global.addEventListener('load', centerAtcText);
  global.setTimeout(centerAtcText, 100);
  global.setTimeout(centerAtcText, 300);
  global.setTimeout(centerAtcText, 800);
  global.setTimeout(centerAtcText, 1500);
})(window);
