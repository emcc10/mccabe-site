...
import { STAGE3E_VARIANTS, variantSettingsLine, type Stage3eVariant } from './variants.js';

export const STAGE3E_GRID = join(DEBUG_DIR, 'stage3e-grid.png');
export const STAGE3E_SPEC = join(DEBUG_DIR, 'stage3e-spec.json');

export interface VariantResult {
  variant: Stage3eVariant;
  outputPath: string;
...
export async function runStage3e(): Promise<{
  grid: string;
  spec: string;
  ...
  for (const v of STAGE3E_VARIANTS) {
  ...
  await writeVariantGrid(STAGE3E_GRID, gridPanels, 2);
  ...
  writeFileSync(
    ...
    JSON.stringify(
      {
        stage: '3E',
        lockedFrom: '3D variant J (believable, not final Bali Silk)',
  ...
  return { grid: STAGE3E_GRID, spec: STAGE3E_SPEC, results };
}
