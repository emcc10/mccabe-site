...
(function (global) {
  ...
  try {
    (function loadBbLeatherSwatchFix() {
      if (!global.document) return;
      if (global.document.querySelector('script[src*="mc-bb-leather-swatch-fix-20260807.js"]')) return;
      var path = String((global.location && global.location.pathname) || "");
      var looksLeather = /bb-faux-leather/i.test(path) || !!global.document.getElementById("beanbag-swatch-wrapper");
      if (!looksLeather) return;
      var s = global.document.createElement("script");
      s.id = "mc-bb-leather-swatch-fix-js";
      s.src = "/v/vspfiles/js/mc-bb-leather-swatch-fix-20260807.js?v=1";
      s.async = true;
...
try {
    (function loadPayLater() {
      ...
      s.src = "/v/vspfiles/js/mc-paypal-pay-later-20260807bbcol7.js?v=1&mcrd=" + Date.now();
      s.async = true;
      (global.document.head || global.document.documentElement).appendChild(s);
    })();
  } catch (ePayLater) {}
  ...
  function ensureSteveSilverHeroPhotoSrc() {
    ...
    /* Hero slot is "-1.jpg".  The template's #mc-ss-piece-hero-boot script owns
       the same <img id="product_photo"> and re-asserts "-1.jpg" from a src
       MutationObserver, so this pass MUST target the same file.  Targeting
       "-2.jpg" here (20260725alex1) made the two owners overwrite each other on
       every mutation record and locked the main thread on SS PDPs. */
    ...
    /* setProductPhotoSrcIfChanged is a no-op when the value already matches.
       Writing an identical src still emits a MutationRecord, which is what fed
       the observer ping-pong, so never write unconditionally here. */
    if (needsSwap) {
      setProductPhotoSrcIfChanged(img, full);
    }
    global.document
      ...
      .forEach(function (link) {
        ...
        if (!href || /-2T\.|-2\.jpg/i.test(href)) {
          setLinkHrefIfChanged(link, full);
        }
      });
    return true;
  }
  ...
  /* The inline template pass (clearAddToCartWrapperBackground) and this file both
     write !important inline styles on the ATC button, each on its own timer, and
     the ATC MutationObserver re-runs this on every style mutation.  Re-writing an
     identical value still emits a MutationRecord, so without this guard the two
     owners repaint each other forever - that is the flicker.  Settle detection is
     done on the computed value because setProperty("#000") serialises to rgb(). */
  function atcChromeSettled(btn) {
    if (!btn) return false;
    try {
  ...
  function forceBlackAtcButton(btn) {
    if (!btn) return;
    if (atcChromeSettled(btn)) return;
    try {
      if (btn.tagName === "INPUT" && (btn.type || "").toLowerCase() === "image") {
        btn.type = "submit";
        btn.removeAttribute("src");
  ...
  function styleCompactAtcButton(wrap) {
    ...
    /* 20260726: this used to early-return when the button was already black.
       That also skipped the structural work below - hiding .mc-cart-icon-wrapper
       and zeroing the wrap padding / button margin - so when the template
       injected the cart glyph AFTER the first pass, it stayed visible and the
  ...
  var saranoniLayoutQuietUntil = 0;
  function markSaranoniUserInteraction() {
    /* After a swatch/size/accordion tap, suppress layout finalize churn that
       rebuilds DOM under the finger and drops the click on mobile. */
    saranoniLayoutQuietUntil = Date.now() + 900;
  }
  function scheduleSaranoniLayoutPass(force) {
    if (!isSaranoniPdpPage() || isStalePdpAuthRun()) return;
    if (!force && Date.now() < saranoniLayoutQuietUntil) return;
    if (force) {
    ...
    saranoniLayoutTimer = global.setTimeout(function () {
      saranoniLayoutTimer = null;
      if (Date.now() < saranoniLayoutQuietUntil) return;
      saranoniLayoutLastRun = Date.now();
      finalizeSaranoniInfoColumnOrder();
    }, 60);
  }

  function setSaranoniAccordionRowOpen(row, open) {
    if (!row) return;
    var header = row.querySelector(".mc-acc-header");
    var panel = row.querySelector(".mc-acc-panel");
  ...
  function toggleSaranoniAccordionRow(row) {
    if (!row) return;
    var wasOpen = row.dataset.open === "1";
    if (row.parentNode) {
      Array.prototype.forEach.call(row.parentNode.querySelectorAll(".mc-acc-row"), function (sibling) {
        setSaranoniAccordionRowOpen(sibling, false);
      });
    }
    if (!wasOpen) setSaranoniAccordionRowOpen(row, true);
  }

  /* 20260727: accordion safety net.
     Three different files build #mc-pdp-accordion (this file's saranoni and bean
     bag passes, and mc-ss-pdp-layout.js on SS bedroom PDPs).  Each installs its
     delegated click listener once and guards with an ATTRIBUTE
     (data-mc-acc-delegated / data-mc-ss-acc).  When a later pass rebuilds or
     clones the accordion, those attributes travel with the new element but the
     listeners do not - so every guard says "already wired" while nothing is
     listening, and tapping FEATURES / PRODUCT DETAILS does nothing.  Seen on
     Highland Park and other SS bedroom products.

     This document-level listener survives every rebuild.  To avoid double
     toggling on pages where a real listener IS attached, it records the row
     state at click time and only acts on the next tick if nothing else changed
     it. */
  (function installAccordionSafetyNet() {
    if (global.__MC_ACC_SAFETY_NET__) return;
    global.__MC_ACC_SAFETY_NET__ = true;
    try {
      global.document.addEventListener(
        "click",
        function (ev) {
          var header =
            ev.target && ev.target.closest ? ev.target.closest(".mc-acc-header") : null;
          if (!header) return;
          var row = header.closest(".mc-acc-row");
          if (!row) return;
          var acc = row.parentNode;
          var before = row.getAttribute("data-open") === "1";
          global.setTimeout(function () {
            try {
              if ((row.getAttribute("data-open") === "1") !== before) return; /* handled */
              if (acc && acc.querySelectorAll) {
                acc.querySelectorAll(".mc-acc-row").forEach(function (other) {
                  other.setAttribute("data-open", "0");
                  var h = other.querySelector(".mc-acc-header");
                  if (h) h.setAttribute("aria-expanded", "false");
                  var pnl = other.querySelector(".mc-acc-panel");
  ...
  function ensureSaranoniAccordionDelegation(acc) {
    if (!acc || acc.dataset.mcAccDelegated === "1") return;
    acc.dataset.mcAccDelegated = "1";
    /* One delegated listener survives row rebuilds — per-header listeners were
       wiped whenever finalize() rewrote accordion innerHTML on mobile. */
    acc.addEventListener("click", function (e) {
      var header =
        e.target && e.target.closest ? e.target.closest(".mc-acc-header") : null;
      if (!header || !acc.contains(header)) return;
      e.preventDefault();
      e.stopPropagation();
      markSaranoniUserInteraction();
      toggleSaranoniAccordionRow(header.closest(".mc-acc-row"));
    });
  }
  ...
  function ensureSaranoniPdpAccordion() {
    ...
    ensureSaranoniAccordionDelegation(acc);
    if (acc.dataset.mcSaranoniRows !== signature) {
      /* Preserve which panel the shopper had open across rebuilds — color/size
         taps used to call finalize() and wipe FEATURES mid-gesture on mobile. */
      var openId = "";
      var openRow = acc.querySelector('.mc-acc-row[data-open="1"]');
      if (openRow) openId = openRow.id || "";
      acc.innerHTML = "";
      ...
      if (openId) {
        var restore = global.document.getElementById(openId);
        if (restore) setSaranoniAccordionRowOpen(restore, true);
      }
    } else {
      rows.forEach(function (row) {
  ...
  function forceCanonicalUnifiedInfoColumnOrder() {
    ...
    try { centerFurnitureUnifiedMainRow(); } catch (eCenterFurn) {}
    try { finalizeColumnPurchaseStack(); } catch (ePurCanon) {}
  }

  /* Furniture / Coaster unified PDPs: center the 650/420 media+info pair.
     Soft goods keep their own row repair — do not touch them here. */
  function centerFurnitureUnifiedMainRow() {
    if (isBeanBagPdpPage() || isCordaroysExtendedPdpPage() || isSaranoniPdpPage()) return;
    if (!isUnifiedAccordionPdp()) return;
    try {
      if (!global.matchMedia || !global.matchMedia("(min-width: 992px)").matches) return;
    } catch (eMq) {
    ...
    var row =
      global.document.querySelector("#v65-product-parent tr.mc-unified-pdp-row") ||
      global.document.querySelector("#v65-product-parent tr.mc-pdp-main-row") ||
      global.document.querySelector("#content_area tr.mc-unified-pdp-row") ||
      global.document.querySelector("#content_area tr.mc-pdp-main-row");
    if (!row) return;
    try {
      ...
      row.style.setProperty("gap", "40px", "important");
      row.style.setProperty("width", "100%", "important");
      row.style.setProperty("max-width", "1120px", "important");
      row.style.setProperty("margin-left", "auto", "important");
      row.style.setProperty("margin-right", "auto", "important");
      row.style.setProperty("box-sizing", "border-box", "important");
    } catch (eRow) {}
    var media = global.document.querySelector(
      "#v65-product-parent td.mc-unified-pdp-media, #v65-product-parent td.mc-pdp-media-td, #v65-product-parent td.vol-product__top--left"
    );
    var info = global.document.querySelector(
      "#v65-product-parent td.mc-unified-pdp-info, #v65-product-parent td.mc-pdp-options-td, #v65-product-parent td.vol-product__top--right"
    );
    if (media) {
      try {
        media.style.setProperty("display", "block", "important");
  ...
  function ensureBeanBagPdpAccordion() {
    ...
    var signature = rows.map(function (row) { return row.id; }).join("|");
    /* Rows here are built by buildSaranoniAccordionRow, which stopped attaching a
       per-header click listener when the binding moved to one delegated container
       listener.  Only ensureSaranoniPdpAccordion was updated to install it, so
       Bean Bag / Cordaroys accordions rendered but never opened.  Idempotent. */
    ensureSaranoniAccordionDelegation(acc);
  ...
  function revealAccordionProductDescription() {
    try {
      ...
      Array.prototype.forEach.call(nodes, function (el) {
        if (!el || !el.style) return;
        /* 20260727: this ran ~970 times on a single page load, rewriting seven
           !important inline properties each time.  Every write invalidates layout,
           so the accordion visibly jumped between top:351 and top:452 nineteen
           times over ~3.5 seconds before settling.  Skip when the element already
           carries the exact state we would write. */
        try {
          var st = el.style;
          if (
            st.getPropertyValue("display") === "block" &&
            st.getPropertyValue("visibility") === "visible" &&
            st.getPropertyValue("opacity") === "1" &&
            st.getPropertyValue("height") === "auto" &&
            st.getPropertyValue("max-height") === "none" &&
            st.getPropertyValue("overflow") === "visible"
          ) {
            return;
          }
        } catch (eSettled) {}
  ...
  function buildDataDrivenSaranoniEntries(select, productCode) {
    ...
    for (i = 0; i < select.options.length; i++) {
      ...
      /* Prefer optionId heroes first (nursery Cover/Sheet/Swaddle use 1364-T.jpg).
  ...
  function findConfiguredColorOption(select, entry) {
    ...
    /* Fallback: match by option value — nursery size labels gain [Additional $N]
       after first paint, which can break slug/label matching. */
    var want = String(entry.optionId || "").trim();
    if (want) {
      for (i = 0; i < select.options.length; i++) {
        if (String(select.options[i].value || "").trim() === want) return select.options[i];
      }
    }
    return null;
  }
  ...
  function saranoniOptionLabelsLookLikeSizes(select) {
    ...
    var sizeRe =
      /^(mini|receiving|toddler|xl|xs|s|m|l|large|small|medium|crib|king|queen|twin|throw|adult|one\s*size|swaddle|quilt|cover|sheet|blanket|muslin)\b/i;
    var hits = 0;
    var n = 0;
    var i;
    for (i = 0; i < select.options.length; i++) {
      ...
      var name = t
        ...
        .replace(/\s+/g, " ")
        .trim();
      /* Nursery kits often use "Muslin/Lush Mini" — treat trailing size tokens. */
      if (sizeRe.test(name) || /\b(mini|quilt|swaddle|cover|sheet|crib|throw|twin|queen|king)\b/i.test(name)) {
        hits += 1;
      }
    }
  ...
  function isSaranoniSizeSelect(select) {
    ...
    /* Category 58 is Saranoni's size axis. Trust it unless labels are styles.
       Nursery kits use Cover/Sheet/Swaddle/Quilt — not Mini/Queen tokens. */
  ...
  /* Capture a known-good hero BEFORE size sync. Volusion price helpers can
     rewrite #product_photo to a missing size-option file (blue ? icon). */
  function captureSaranoniSafeHeroSrc() {
    var mainImg = global.document.getElementById("product_photo");
    var cur = "";
    try {
      cur = mainImg ? String(mainImg.getAttribute("src") || mainImg.src || "") : "";
    } catch (eCur) {}
    if (configuredColorActiveSrc && !isSaranoniBadHeroSrc(configuredColorActiveSrc)) {
      return configuredColorActiveSrc;
    }
    if (configuredColorDefaultSrc && !isSaranoniBadHeroSrc(configuredColorDefaultSrc)) {
      return configuredColorDefaultSrc;
    }
    if (cur && !isSaranoniBadHeroSrc(cur) && mainImg && mainImg.naturalWidth > 1) {
      return cur;
    }
    return cur && !isSaranoniBadHeroSrc(cur) ? cur : "";
  }

  function buildSaranoniSizeHeroCandidateFiles(productCode, entry, colorEntry) {
    var files = [];
    function push(name) {
    ...
    var sizeSlug = entry.labelSlug || saranoniLabelSlug(entry.label);
    var colorSlug = colorEntry
      ? colorEntry.labelSlug || saranoniLabelSlug(colorEntry.label)
      : "";
    /* Baby Bamboni Lite Sets (and similar) ship as SIZE--COLOR composites. */
    if (productCode && sizeSlug && colorSlug) {
      push(productCode + "-" + sizeSlug + "--" + colorSlug + "-T.jpg");
      push(productCode + "-" + sizeSlug + "--" + colorSlug + "-S.jpg");
    }
    push(entry.mainImage);
    push(entry.mainImageAlt);
    push(entry.swatchImage);
    push(entry.swatchImageAlt);
    if (productCode && sizeSlug) {
      push(productCode + "-" + sizeSlug + "-T.jpg");
      push(productCode + "-" + sizeSlug + "-S.jpg");
    }
    return files;
  }

  function applySaranoniSizeHeroPhoto(productCode, entry, colorEntry, safePrevSrc) {
    if (!productCode || !entry) return;
    var mainImg = global.document.getElementById("product_photo");
    var prevSrc = String(safePrevSrc || "").trim() || captureSaranoniSafeHeroSrc();
    var label = entry.label || "";
    var files = buildSaranoniSizeHeroCandidateFiles(productCode, entry, colorEntry);
    var token = String(Date.now()) + ":size:" + String(entry.optionId || "");
    saranoniSizeActiveImageFile = token;

    function restoreSafeHero() {
      if (!mainImg) {
        dismissSaranoniProductPhotoLoading();
        return;
      }
      var fallback = prevSrc || configuredColorActiveSrc || configuredColorDefaultSrc || "";
      if (fallback) {
        try {
          var cur = String(mainImg.getAttribute("src") || mainImg.src || "");
          var broken = !mainImg.naturalWidth || mainImg.naturalWidth < 2;
          if (broken || cur !== fallback) {
            setSaranoniSizePhotoSrc(fallback, label, productCode);
          }
        } catch (eRest) {}
      }
      dismissSaranoniProductPhotoLoading();
    ...
    function tryAt(idx) {
      ...
      var f = files[idx];
      loadProductScopedColorImage(productCode, f, function (resolvedSrc) {
        if (saranoniSizeActiveImageFile !== token) return;
        if (resolvedSrc) {
          saranoniSizeActiveImageFile = f;
          /* Keep color-enforce from reverting a valid size/color composite. */
          configuredColorActiveSrc = resolvedSrc;
          configuredColorEnforceUntil = Date.now() + 8000;
          setSaranoniSizePhotoSrc(resolvedSrc, label, productCode);
          scheduleSaranoniSizeHeroLock(productCode, f, label);
          global.setTimeout(function () {
            if (saranoniSizeActiveImageFile !== f) return;
            if (
              mainImg &&
              mainImg.complete &&
              (!mainImg.naturalWidth || mainImg.naturalWidth < 2)
            ) {
              restoreSafeHero();
            }
          }, 200);
  ...
  function formatSaranoniAdditionalDollars(amount) {
    var n = Number(amount) || 0;
    if (n <= 0) return "";
    return n % 1 ? n.toFixed(2) : String(Math.round(n));
  }

  /* Keep/restore "Additional $N" under size thumbs when Volusion option text has it. */
  function refreshSaranoniSizeThumbAdditionalPrices(ctx) {
    if (!ctx || !ctx.select || !ctx.entries) return;
    ctx.entries.forEach(function (entry) {
      var opt = findConfiguredColorOption(ctx.select, entry);
      if (!opt && ctx.select && ctx.select.options) {
        var want = String(entry.optionId || "").trim();
        var i;
        for (i = 0; i < ctx.select.options.length; i++) {
          if (String(ctx.select.options[i].value || "").trim() === want) {
            opt = ctx.select.options[i];
            break;
          }
        }
      }
      var btn = global.document.querySelector(
        '.mc-saranoni-size-thumb[data-option-id="' + String(entry.optionId || "") + '"]'
      );
      if (!btn) return;
      var liveText = String((opt && opt.text) || entry.label || "");
      /* Also scan the select by value in case entry.label is stale. */
      if (opt && opt.text) liveText = String(opt.text);
      var parts = splitSaranoniSizeDisplayLabel(liveText);
      var labelEl = btn.querySelector(".mc-saranoni-size-thumb__label");
      if (labelEl && parts.name) labelEl.textContent = parts.name;
      var priceEl = btn.querySelector(".mc-saranoni-size-thumb__price");
      var dollars = formatSaranoniAdditionalDollars(parts.additional);
      var aria =
        parts.additional > 0 ? parts.name + " (Additional $" + dollars + ")" : parts.name;
      btn.setAttribute("aria-label", aria);
      btn.setAttribute("title", aria);
      if (parts.additional > 0) {
        if (!priceEl) {
          priceEl = global.document.createElement("span");
          priceEl.className = "mc-saranoni-size-thumb__price";
          btn.appendChild(priceEl);
        }
        priceEl.textContent = "Additional $" + dollars;
        try {
    ...
    /* Keep signature honest so a later pass rebuilds if needed. */
    try {
      var row = global.document.getElementById("mc-saranoni-size-thumbs");
      if (row && ctx.productCode) {
        row.setAttribute(
          "data-mc-signature",
          ctx.productCode +
            "|" +
            ctx.entries
              .map(function (e) {
                var o = findConfiguredColorOption(ctx.select, e);
                var t = String((o && o.text) || e.label || "");
                return e.optionId + ":" + extractAdditionalFromOptionText(t);
              })
              .join(",")
        );
      }
  ...
  function scheduleSaranoniSizeAdditionalPriceRefresh(ctx) {
    if (!ctx) return;
    [0, 200, 600, 1200, 2500].forEach(function (delay) {
      global.setTimeout(function () {
        try {
          refreshSaranoniSizeThumbAdditionalPrices(ctx);
        } catch (eDelay) {}
      }, delay);
    });
  }

  function clearSaranoniAltViewHeroHold() {
    try {
      delete global.__MC_PDP_ALT_VIEW_ACTIVE_SRC__;
      delete global.__MC_PDP_ALT_VIEW_ACTIVE_AT__;
    } catch (eClr) {
      try {
        global.__MC_PDP_ALT_VIEW_ACTIVE_SRC__ = "";
        global.__MC_PDP_ALT_VIEW_ACTIVE_AT__ = 0;
      } catch (eClr2) {}
    }
  }
  ...
  function applySaranoniSizeVariantSelection(ctx, entry, updateHero) {
    ...
    /* Snapshot a good hero before Volusion helpers can blank #product_photo. */
    var safePrevSrc = captureSaranoniSafeHeroSrc();
    clearSaranoniAltViewHeroHold();
    var colorCtx = findConfiguredColorSwatchContext();
    var colorEntry =
      configuredColorActiveEntry ||
      (colorCtx ? readConfiguredColorSelectEntry(colorCtx) : null);
    var opt = findConfiguredColorOption(ctx.select, entry);
    if (opt) {
    ...
    if (updateHero !== false) {
      applySaranoniSizeHeroPhoto(ctx.productCode, entry, colorEntry, safePrevSrc);
    }
    try {
      ensurePdpTitleInOptionsColumn();
    } catch (eTitleKeep) {}
  ...
  function ensureSaranoniSizeVariantThumbs(ctx) {
    ...
    var signature =
      ...
      ctx.entries
        .map(function (e) {
          var liveOpt = findConfiguredColorOption(ctx.select, e);
          var liveText = String((liveOpt && liveOpt.text) || e.label || "");
          return e.optionId + ":" + extractAdditionalFromOptionText(liveText);
        })
        .join(",");
    ...
    ctx.entries.forEach(function (entry) {
      ...
      /* Always prefer the live select option text — that is where Volusion puts [Additional $N]. */
      var parts = splitSaranoniSizeDisplayLabel(opt.text || entry.label);
      var aria =
        parts.additional > 0
  ...
  function ensureSaranoniVariantUi() {
    ...
    if (sizeCtx) {
      try {
        refreshSaranoniSizeThumbAdditionalPrices(sizeCtx);
        scheduleSaranoniSizeAdditionalPriceRefresh(sizeCtx);
      } catch (eSarPrice) {}
    }
  ...
  function applyConfiguredColorMainPhoto(fileName, label, productCode) {
    ...
    /* When a size is already chosen, prefer SIZE--COLOR composites first. */
    var preferFiles = [];
    if (pc && saranoniSizeActiveOptionId && configuredColorActiveEntry) {
      var sizeCtx = findSaranoniSizeVariantContext();
      var sizeEntry = sizeCtx
        ? resolveConfiguredColorEntry(sizeCtx, saranoniSizeActiveOptionId)
        : null;
      if (sizeEntry) {
        preferFiles = buildSaranoniSizeHeroCandidateFiles(
          pc,
          sizeEntry,
          configuredColorActiveEntry
        ).filter(function (f) {
          return String(f).indexOf("--") !== -1;
        });
      }
    }
    ...
    if (
      ...
      preferFiles.indexOf(configuredColorActiveEntry.mainImageAlt) === -1
    ) {
      preferFiles.push(configuredColorActiveEntry.mainImageAlt);
    }

    function applyResolved(resolvedSrc) {
      ...
      if (
        ...
        !configuredColorImageMatchesOption(
          ...
          configuredColorActiveEntry.label || configuredColorActiveEntry.labelSlug
        ) &&
        !(
    ...
    function tryPrefer(idx) {
      if (global.__MC_CONFIGURED_COLOR_IMAGE_TOKEN__ !== token) return;
      if (idx >= preferFiles.length) {
        applyResolved("");
        return;
      }
      loadProductScopedColorImage(pc, preferFiles[idx], function (resolvedSrc) {
        if (global.__MC_CONFIGURED_COLOR_IMAGE_TOKEN__ !== token) return;
        if (resolvedSrc) {
          applyResolved(resolvedSrc);
          return;
        }
        tryPrefer(idx + 1);
      });
  ...
  function enforceConfiguredColorPhoto(productCode) {
    ...
    configuredColorEnforceTimer = global.setInterval(function () {
      ...
      if (altOverride && configuredColorImageBelongsToProduct(altOverride, pc)) {
        /* Never re-pin a broken size-option URL latched by alt-view hold. */
        var altFile = saranoniPhotoFileName(altOverride);
        var altLooksSizeOnly =
          !!saranoniSizeActiveOptionId &&
          isSaranoniColorOptionPhotoFile(altFile) &&
          configuredColorImageMatchesOption(altOverride, pc, saranoniSizeActiveOptionId);
        if (altLooksSizeOnly || isSaranoniBadHeroSrc(altOverride)) {
          clearSaranoniAltViewHeroHold();
        } else {
  ...
  function syncConfiguredColorSelect(select, opt) {
    ...
    /* Skip Volusion change_option on Saranoni — it can blank #product_photo
       (blue ?) while our CDN option photos are still probing. Cart value is
       kept via ensureConfiguredColorCartField + the select itself. */
    var isSarColor = /^SAR/i.test(parseProductCodeFromSelectName(select.name));
    if (!isSarColor && typeof global.change_option === "function") {
      try {
  ...
  function syncConfiguredColorSwatchUi(ctx, applyPhoto, forcePhoto) {
    ...
    var wrap = global.document.getElementById("mc-configured-color-swatch-wrapper");
    var buttons = wrap
      ? wrap.querySelectorAll(".mc-configured-color-swatch")
      : global.document.querySelectorAll(".mc-configured-color-swatch");
  ...
  var BB_COVER_ASSET_BY_LABEL = {
    ...
    "faux leather|black": ["https://cordaroys.com/cdn/shop/files/FC-CW-BK.jpg", "/v/vspfiles/photos/bb-fauxLeather-black.jpg"],
  ...
  function bbCoverAsset(label) {
    ...
    /* Prefer the swatch label family over product code so a wrong baked URL
       (e.g. faux-fur-black on Faux Leather / Black) still maps correctly. */
    var family = "faux fur";
    if (/faux\s*leather/i.test(normalized)) {
      family = "faux leather";
    } else if (/chenille/i.test(normalized)) {
      family = "chenille";
    } else if (/faux\s*fur/i.test(normalized)) {
      family = "faux fur";
    } else if (/FAUX-?LEATHER/i.test(code) || /BB-FAUX(?!-FUR)/i.test(code)) {
      family = "faux leather";
    } else if (/CHENILLE/i.test(code)) {
      family = "chenille";
    } else if (/FAUX-?FUR/i.test(code)) {
      family = "faux fur";
    }
  ...
  function repairBeanBagCoverSwatchThumbs() {
    if (!isBeanBagPdpPage()) return;
    var wrap = global.document.getElementById("beanbag-swatch-wrapper");
    if (!wrap) return;
    wrap.querySelectorAll("img.beanbag-swatch").forEach(function (swatch) {
      ...
      var cur = String(swatch.getAttribute("src") || swatch.src || "");
      var want = String(asset.thumb);
      var curBase = cur.split("?")[0];
      var wantBase = want.split("?")[0];
      var wrongFamily =
        /faux\s*leather/i.test(label) && /faux-?fur/i.test(curBase);
      if (wrongFamily || curBase.indexOf(wantBase) === -1) {
        try {
          var stamped =
            want.indexOf("?") >= 0 ? want : want + "?v=20260807bbcol3";
          swatch.removeAttribute("srcset");
          swatch.setAttribute("src", stamped);
          swatch.src = stamped;
        } catch (eThumb) {}
      }
    });
  }
  global.__MC_BB_REPAIR_SWATCH_THUMBS__ = repairBeanBagCoverSwatchThumbs;

  /* MC_BB_LEATHER_INLINE_20260807 — do not wait on a second script file. */
  (function inlineBbLeatherThumbFix() {
    function fixLeatherThumbs() {
      try {
        if (!isBeanBagPdpPage()) return;
        var wrap = global.document.getElementById("beanbag-swatch-wrapper");
        if (!wrap) return;
        ...
        wrap.querySelectorAll("img.beanbag-swatch").forEach(function (img) {
          ...
          var color = label.split("/").pop().replace(/faux\s*leather/i, "").replace(/\s+/g, " ").trim().toLowerCase();
          var want = map[color];
          if (!want) return;
          var cur = String(img.getAttribute("src") || img.src || "");
          if (/faux-?fur/i.test(cur) || cur.indexOf(want.split("?")[0]) === -1) {
            img.removeAttribute("srcset");
            img.setAttribute("src", want + "?v=20260807inline1");
            img.src = want + "?v=20260807inline1";
          }
        });
      } catch (eInlineLeather) {}
    }
    [0, 100, 400, 900, 1800, 3500, 7000].forEach(function (ms) {
      global.setTimeout(fixLeatherThumbs, ms);
    });
  })();
  ...
  function initBeanBagImageSync() {
    if (!isBeanBagPdpPage()) return;
    /* Always repair leather thumbs — a stale CF form.js?mcrd= can latch
       mcBbImgBound before this impl loads and leave faux-fur on leather black. */
    try {
      repairBeanBagCoverSwatchThumbs();
    } catch (eBbThumbs0) {}
    [0, 200, 800, 1600, 3200, 6000].forEach(function (ms) {
      global.setTimeout(function () {
        try {
          repairBeanBagCoverSwatchThumbs();
        } catch (eBbThumbs) {}
...
function revealBeanBagRelated() {
  ...
  function extractAdditionalFromOptionText(text) {
    ...
    /* Parse the capture directly — parseMoney() requires a leading "$" and would
       turn bare "7" / "17.00" into 0 (which wiped every Saranoni size Additional). */
    if (!m) return 0;
  ...
  function ensureBedroomCollectionSection() {
    ...
    try {
      ...
      var s = global.document.createElement("script");
      s.src =
        "/v/vspfiles/js/mc-bedroom-collection-section.js?v=20260807colpdp1&mcrd=" +
        Date.now();
      s.async = true;
      s.onload = function () {
  ...
  if (!global.__MC_PDP_AUTH_CTA_CAPTURE__) {
    ...
    function handleSaranoniSizeThumbClick(btn) {
      if (!btn || !isSaranoniPdpPage()) return false;
      var optionId = btn.getAttribute("data-option-id");
      if (!optionId) return false;
      var ctx = findSaranoniSizeVariantContext();
      if (!ctx) return false;
      var entry = resolveConfiguredColorEntry(ctx, optionId);
      if (!entry) return false;
      applySaranoniSizeVariantSelection(ctx, entry, true);
    ...
    global.document.addEventListener(
      ...
      function (e) {
        ...
        var sizeThumb =
          e.target && e.target.closest ? e.target.closest(".mc-saranoni-size-thumb") : null;
        if (sizeThumb) {
          e.preventDefault();
          e.stopPropagation();
          e.stopImmediatePropagation();
...
(function (g, d) {
  ...
  function repairBeanBagSwatches() {
    ...
    wrap.querySelectorAll("img.beanbag-swatch").forEach(function (swatch) {
      ...
      if (asset && asset.thumb) {
        var curBase = String(src || "").split("?")[0];
        var wantBase = String(asset.thumb).split("?")[0];
        var wrongFamily = /faux\s*leather/i.test(label) && /faux-?fur/i.test(curBase);
        if (wrongFamily || curBase.indexOf(wantBase) === -1) {
          swatch.setAttribute("src", asset.thumb);
          try {
            swatch.src = asset.thumb;
            swatch.removeAttribute("srcset");
          } catch (eSetThumb) {}
          src = asset.thumb;
        }
  ...
  function runRepair() {
    ...
    try {
      if (typeof g.__MC_BB_REPAIR_SWATCH_THUMBS__ === "function") g.__MC_BB_REPAIR_SWATCH_THUMBS__();
    } catch (eBbThumbsRepair) {}
  ...
  [200, 800, 1600, 3200].forEach(function (ms) {
...
(function (g, d) {
  ...
  function restore() {
    ...
    var hero = d.getElementById("product_photo");
    var root = d.getElementById("v65-product-parent");
    /* SAR-/Saranoni: mc-pdp-alt-view-row owns the gallery. Building -N thumbs
       here races hideSaranoniHeroAltviews() — each image.onload set
       #altviews to display:flex while the hide path set display:none, which
       visitors saw as continuous alt-row flashing (Peter Rabbit muslin, etc.). */
    if (!root || !productCode || !hero || /^(?:TMH-|BB-|SS-|SAR-)/.test(productCode)) return;
