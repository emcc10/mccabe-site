/* MC_BEDROOM_COLLECTION_SECTION_20260620 / PDP-only gate 20260807colpdp1 */
!function(w,d){
  'use strict';

  function isProductDetailPage(){
    try{
      if(d.body&&(d.body.classList.contains('productdetails')||d.body.classList.contains('mc-product-page')))return!0;
      if(d.querySelector&&d.querySelector('#v65-product-parent')&&d.querySelector('input[name="ProductCode"],input[name="productcode"]'))return!0;
      var p=String(w.location.pathname||'').toLowerCase();
      return/\/product-p\//.test(p)||/productdetails\.asp/.test(p);
    }catch(_){return!1}
  }
  function removeStrayCollectionOnNonPdp(){
    if(isProductDetailPage())return;
    var sec=d.getElementById('mc-bedroom-collection');
    if(sec&&sec.parentNode){
      try{sec.parentNode.removeChild(sec)}catch(_){}
    }
  }
  /* Always strip on category/listing pages — even if an older latched copy already ran. */
  removeStrayCollectionOnNonPdp();
  [0,400,1200,2500,5000].forEach(function(ms){setTimeout(removeStrayCollectionOnNonPdp,ms)});

  /* BB faux-leather Black was stuck on faux-fur because CF cached a fat form.js?mcrd=.
     Load a tiny unique fixer that only remaps leather cover thumbs. */
  try{
    var onBbLeather=/bb-faux-leather/i.test(String(w.location.pathname||''))||!!d.getElementById('beanbag-swatch-wrapper');
    if(onBbLeather&&!d.querySelector('script[src*="mc-bb-leather-swatch-fix-20260807.js"]')&&!w.__MC_BB_LEATHER_SWATCH_FIX_LOADING__){
      w.__MC_BB_LEATHER_SWATCH_FIX_LOADING__=!0;
  ...
  if(!w||!d||w.__MC_BEDROOM_COLLECTION_SECTION_20260807colpdp1__)return;
  w.__MC_BEDROOM_COLLECTION_SECTION_20260807colpdp1__=!0;
  try{
    w.__MC_BEDROOM_COLLECTION_SECTION_20260620__=!0;
    w.__MC_BEDROOM_COLLECTION_SECTION_20260724coll1__=!0;
  }catch(_){}
  ...
  function findImage(anchor,card){
    ...
    return imageFromProductHref(anchor&&anchor.getAttribute('href'));
  }
  function imageFromProductHref(href){
    var m=String(href||'').match(/product-p\/([^\/?#.]+)/i);
    if(!m||!m[1])return'';
    var code=String(m[1]).toUpperCase();
    return '/v/vspfiles/photos/'+code+'-1T.jpg';
  }
    ...
    products.forEach(function(product){
      ...
      if(product.image){
        ...
        img.onerror=function(){
          var full=String(product.image||'').replace(/-1T\.jpg/i,'-1.jpg');
          if(full&&full!==product.image){ img.onerror=null; img.src=full; }
          else if(image.parentNode){ image.style.display='none'; }
        };
        image.appendChild(img);
      } else {
        var fallback=imageFromProductHref(product.href);
        if(fallback){
          ...
          img2.onerror=function(){
            var full=fallback.replace(/-1T\.jpg/i,'-1.jpg');
            if(full!==fallback){ img2.onerror=null; img2.src=full; }
          };
  ...
  function run(){
    /* PDP-only: on category/listing pages currentName() can latch onto a grid
       product title (e.g. Cassie bed on Beds) and inject "The Collection". */
