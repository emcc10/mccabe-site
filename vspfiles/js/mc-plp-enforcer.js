...
(function (global) {
  ...
  function upgradePdpAuthCtaForm() {
    try {
      ...
      global.document.documentElement.setAttribute("data-mc-pdp-auth-bbcol4", "1");
      global.document
        .querySelectorAll('script[src*="mc-pdp-auth-cta-form"]')
        .forEach(function (old) {
          var src = String(old.getAttribute("src") || "");
          if (/mc-pdp-auth-cta-form-20260807bbcol4\.js/i.test(src)) return;
          try {
            old.remove();
          } catch (eRm) {}
        });
  ...
  function loadBbLeatherSwatchFix() {
    try {
      ...
      var code = String((codeEl && codeEl.value) || global.global_Current_ProductCode || "");
      var onLeather =
        /bb-faux-leather/i.test(path) ||
        /FAUX-?LEATHER/i.test(code) ||
        /BB-FAUX(?!-FUR)/i.test(code) ||
  ...
  function upgradeAltViewRow() {
    /* Auth stub owns the single flash7 alt-view load - PLP must not inject.
       ...
       re-injecting mc-pdp-alt-view-row.js (Wearable / Peter Rabbit flash). */
    try {
  ...
  /* Bedroom "The Collection" is PDP-only. On Beds/category pages it latches onto
     a grid product title (e.g. Cassie) and incorrectly injects the strip. */
  function stripBedroomCollectionOnCategory() {
    if (!isCategoryPlp()) return;
    try {
      var sec = global.document.getElementById("mc-bedroom-collection");
  ...
  function run() {
    ...
    if (!isCategoryPlp()) return;
    markCategory();
    stripBedroomCollectionOnCategory();
