...
(function () {
  ...
  window.MTL_RENDERER_BUILD = "sectional-20260626-top-price-panel-v31";
  ...
  function findFinancingBlockAfterTitle(title) {
    ...
    for (i = 0; i < nodes.length; i++) {
      ...
      if (!best || node.compareDocumentPosition(best) & Node.DOCUMENT_POSITION_FOLLOWING) {
        best = node;
      }
    }
    return best;
  ...
  function assertTopPricePanelOrder(panel) {
  ...
  function hidePricingBeforeTitleWrap() {
    ...
    while (child && child !== wrap) {
      if (child.nodeType === 1) {
        if (
          child.classList &&
          (child.classList.contains("mc-pdp-retail-row") ||
            child.classList.contains("mc-pdp-member-pricing") ||
            child.classList.contains("colors_pricebox"))
        ) {
          hidePricingNode(child);
          child.querySelectorAll(
            ".mc-pdp-retail-row, .mc-pdp-member-pricing, .mc-pdp-retail-label, .product_list_price, .product_productprice"
          ).forEach(hidePricingNode);
        } else {
          child.querySelectorAll(".mc-pdp-retail-row, .mc-pdp-member-pricing, .mc-pdp-retail-label").forEach(
            function (node) {
              if (!nodeIsFinancingEl(node)) hidePricingNode(node);
            }
          );
        }
      }
  ...
  function readRetailAmountForTopPanel() {
    ...
    var existing = document.querySelector("#mc-pdp-top-price-panel .mtl-top-price__amount");
    if (existing) {
      var a = parseMoneyAmount(existing.textContent || "");
      if (a > 0) return a;
    }
  ...
  function hideNativeVolusionSaleUnderTitle() {
    ...
    root
      .querySelectorAll(
        ".colors_pricebox .product_saleprice, .colors_pricebox .product_sale_price, .colors_pricebox font.product_sale_price"
      )
      .forEach(function (node) {
        if (!node || (top.contains && top.contains(node))) return;
        try {
          node.style.setProperty("display", "none", "important");
          node.style.setProperty("visibility", "hidden", "important");
          node.style.setProperty("height", "0", "important");
  ...
  function suppressExternalTopPricePanels() {
    ...
    var legacyTop = document.getElementById("mtl-pdp-top-price");
    if (legacyTop) {
      try {
        legacyTop.style.setProperty("display", "none", "important");
      } catch (eLeg) {}
    }
    ...
    window.__MTL_OWNS_TOP_PRICE__ = "20260601a";
  }

  var __mtlTopPriceOrderObs = null;
  function ensureTopPriceOrderObserver() {
    if (__mtlTopPriceOrderObs || typeof MutationObserver === "undefined") return;
    var wrap = findTitleRightWrap();
    var parent = wrap && wrap.parentNode;
    if (!parent) return;
    __mtlTopPriceOrderObs = new MutationObserver(function () {
  ...
  function ensureTopPricePanel() {
    ...
    var panel = document.getElementById("mc-pdp-top-price-panel");
    if (!panel) {
      var legacy = document.getElementById("mtl-pdp-top-price");
      if (legacy) {
        legacy.id = "mc-pdp-top-price-panel";
        legacy.className = "mc-pdp-top-price-panel";
        panel = legacy;
      }
    }
    if (!panel) {
      panel = document.createElement("div");
      panel.id = "mc-pdp-top-price-panel";
      panel.className = "mc-pdp-top-price-panel";
      panel.setAttribute("data-mtl-generated", "true");
      panel.setAttribute("data-mtl-top-price-panel", "1");
    }
    return panel;
  }
  ...
  function mountTopPricePanelUnderTitleOnce() {
    ...
    if (!panel) return;
    if (!isTopPricePanelMounted(panel)) {
      if (panel.parentNode) {
        try {
      ...
      if (!assertTopPricePanelOrder(panel) && !insertTopPricePanel(panel)) {
        window.__MTL_TOP_PRICE_MOUNT_GAVE_UP__ = true;
        return;
      }
    }
    try {
      assertTopPricePanelOrder(panel);
      panel.dataset.mtlTopPriceMounted = "1";
  ...
  function ensureProductSummaryInitialPlacement(sum, section) {
    if (!sum || sum.parentNode) return;
    if (section && section.parentNode) {
      try {
        section.parentNode.insertBefore(sum, section.nextSibling);
      } catch (ePl) {
        console.warn("[MTL] ensureProductSummaryInitialPlacement", ePl);
      }
    }
  }
  ...
  function mtlHideLegacyVolusionPriceDuplicatesForSectional() {
    ...
    if (!document.querySelector("#mc-pdp-top-price-panel, .mtl-product-price-block")) return;
    document
      .querySelectorAll(
  ...
  function ensureProductSummary(section) {
    ...
    ensureProductSummaryInitialPlacement(sum, section);
    updateTopPricePanel();
...
/* PLP/site fixes: design-toolkit.min.js (category pages). Volusion SFTP caps /vspfiles/js/*.js at 128KB — this file is ~175KB. */
