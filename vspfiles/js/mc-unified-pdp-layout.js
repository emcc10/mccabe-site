...
(function (global) {
  ...
  function ensureBedroomCollectionSection() {
    if (!isPDP()) return;
    if (global.__MC_BEDROOM_COLLECTION_SECTION_LOADING__ || global.__MC_BEDROOM_COLLECTION_SECTION_20260723mob1__) return;
    if (qs("script[src*='mc-bedroom-collection-section.js'][src*='20260723mob1']")) return;
    try {
      global.__MC_BEDROOM_COLLECTION_SECTION_LOADING__ = true;
      var s = global.document.createElement("script");
      s.src = "/v/vspfiles/js/mc-bedroom-collection-section.js?v=20260723mob1&mcrd=" + Date.now();
      s.async = true;
      s.onload = function () {
  ...
  function normalizeMediaColumn(mediaTd) {
    ...
    if (img && img.style) {
      ...
      if (imgNormVer !== LAYOUT_VER) {
        ...
        if (isSteveSilver) {
          img.style.setProperty("width", isDesktop ? "700px" : "100%", "important");
